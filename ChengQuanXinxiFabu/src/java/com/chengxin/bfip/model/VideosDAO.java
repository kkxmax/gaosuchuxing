...
public class VideosDAO extends BaseDataAccessObject {

	private static final String TABLE_NAME = "Videos";
	private static final String TABLE_VIEW = "Videos_v";

	public void insert(Videos member) {
		DBModelUtil.processSecure(Videos.class.getName(), member, DBModelUtil.C_SECURE_TYPE_ENCRYPT);

		this.getHibernateTemplate().save(member);
	}

	public void update(Videos member) {
		DBModelUtil.processSecure(Videos.class.getName(), member, DBModelUtil.C_SECURE_TYPE_ENCRYPT);

		this.getHibernateTemplate().update(member);
	}

	public void delete(Videos member) {
		DBModelUtil.processSecure(Videos.class.getName(), member, DBModelUtil.C_SECURE_TYPE_ENCRYPT);
	...
	public Videos get(int id) {
		...
		if (list.size() > 0) {
			Videos member = (Videos)list.get(0);

			DBModelUtil.processSecure(Videos.class.getName(), member, DBModelUtil.C_SECURE_TYPE_DECRYPT);
	...
	public Videos getDetail(int id) {
		...
		List<Videos> resultList = this.search(filterParamObject);
	...
	public List<Videos> search(JSONObject filterParamObject) {
		return search(filterParamObject, "");
	}

	public List<Videos> search(JSONObject filterParamObject, String extraWhere) {
		Session session = SessionFactoryUtils.getNewSession(this.getHibernateTemplate().getSessionFactory());
		Transaction transaction = session.beginTransaction();
		...
		List<Videos> resultList = new ArrayList<Videos>();
		...
		if (queryList != null) {
			...
			for(int i = 0; i < listSize; i++) {
				...
				row.setWriteTime(DateTimeUtil.dateFormat(DateTimeUtil.stringToDate(CommonUtil.toStringDefault(objectArray[6])), "yyyy.MM.dd HH:mm"));
				row.setCarouselId(CommonUtil.toIntDefault(objectArray[7]));

				DBModelUtil.processSecure(Videos.class.getName(), row, DBModelUtil.C_SECURE_TYPE_DECRYPT);
