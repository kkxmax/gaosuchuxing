 ...
 */
public class XyleixingsDAO extends BaseDataAccessObject {

    private static String VIEW = "Xyleixings_v";
    
    //张号类型
    public static final int Xyleixings_ST_UP = 0;
    public static final int Xyleixings_ST_DOWN = 2;
    
    
    public void insert(Xyleixings member) {
        DBModelUtil.processSecure(Xyleixings.class.getName(), member, DBModelUtil.C_SECURE_TYPE_ENCRYPT);
        
        this.getHibernateTemplate().save(member);
    }

    public void update(Xyleixings member) {
        DBModelUtil.processSecure(Xyleixings.class.getName(), member, DBModelUtil.C_SECURE_TYPE_ENCRYPT);
        
        this.getHibernateTemplate().update(member);
    }
    
    public void delete(Xyleixings member) {
        DBModelUtil.processSecure(Xyleixings.class.getName(), member, DBModelUtil.C_SECURE_TYPE_ENCRYPT);
    ...
    public Xyleixings get(int id) {
        StringBuffer stringBuffer = new StringBuffer();

        stringBuffer.append("FROM Xyleixings ");
        ...
        if (list.size() > 0) {
        	Xyleixings member = (Xyleixings)list.get(0);
            
            DBModelUtil.processSecure(Xyleixings.class.getName(), member, DBModelUtil.C_SECURE_TYPE_DECRYPT);
    ...
    public Xyleixings get(String where) {
    	 List<Xyleixings> resultList = this.search(null, where, "");
    ...
    public Xyleixings getDetail(int id) {
        ...
        List<Xyleixings> resultList = this.search(filterParamObject);
    ...
    private SQLWithParameters _makeSearchQuery(boolean isCountSQL, JSONObject filterParamObject, String extraWhere, String extraOrder) {
        ...
        if(isCountSQL) {
            ...
            sql.appendSQL("SELECT id , title , upper_id , account_id , write_time ");
        }
    ...
    public List<Xyleixings> search(JSONObject filterParamObject) {
    	return search(filterParamObject, "", "");
    }
    
    public List<Xyleixings> search(JSONObject filterParamObject, String extraWhere) {
    	return search(filterParamObject, extraWhere, "");
    }
    
    public List<Xyleixings> search(JSONObject filterParamObject, String extraWhere, String extraOrder) {
        ...
        List<Xyleixings> resultList = new ArrayList<Xyleixings>();
        ...
        if (queryList != null) {
            ...
            for(int i = 0; i < listSize; i++) {
                ...
                Xyleixings row = new Xyleixings();
                
                row.setId(CommonUtil.toIntDefault(objectArray[0]));
                row.setTitle(CommonUtil.toStringDefault(objectArray[1]));
                row.setUpper_id(CommonUtil.toIntDefault(objectArray[2]));
                row.setAccount_id(CommonUtil.toIntDefault(objectArray[3]));
                row.setWrite_time(DateTimeUtil.stringToDate(CommonUtil.toStringDefault(objectArray[4])));
                
                DBModelUtil.processSecure(Xyleixings.class.getName(), row, DBModelUtil.C_SECURE_TYPE_DECRYPT);
