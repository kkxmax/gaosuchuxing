...
public class EstimatesDAO extends BaseDataAccessObject {
	...
	private SQLWithParameters _makeSearchQuery(boolean isCountSQL, JSONObject filterParamObject, String extraWhere) {
		...
		if(isCountSQL) {
			...
			sql.appendSQL("SELECT id , booknum , name , content , kind , method , reason , update_time , account_id , write_time , kind_name , method_name ");
		}
	...
	public List<Estimates> search(JSONObject filterParamObject, String extraWhere) {
		...
		if (queryList != null) {
			...
			for(int i = 0; i < listSize; i++) {
				...
				row.setContent(CommonUtil.toStringDefault(objectArray[3]));
				row.setKind(CommonUtil.toIntDefault(objectArray[4]));
				row.setMethod(CommonUtil.toIntDefault(objectArray[5]));
				row.setReason(CommonUtil.toStringDefault(objectArray[6]));
				row.setUpdate_time(CommonUtil.toStringDefault(objectArray[7]));
				row.setAccount_id(CommonUtil.toIntDefault(objectArray[8]));
				row.setWrite_time(CommonUtil.toStringDefault(objectArray[9]));
				row.setKind_name(CommonUtil.toStringDefault(objectArray[10]));
				row.setMethod_name(CommonUtil.toStringDefault(objectArray[11]));
