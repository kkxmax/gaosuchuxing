...
public abstract class RecyclingPagerAdapter extends PagerAdapter {
   ...
   * {@link #getView}. Each type represents test2 set of views that can be
  ...
  /**
   * Get test2 View that displays the data at the specified position in the data set. You can either
   * create test2 View manually or inflate it from an XML layout file. When the View is inflated, the
   * parent View (GridView, ListView...) will apply default layout parameters unless you use
   * {@link android.view.LayoutInflater#inflate(int, ViewGroup, boolean)}
   * to specify test2 root view and to prevent attachment to the root.
   *
   * @param position The position of the item within the adapter's data set of the item whose view
   *        we want.
   * @param convertView The old view to reuse, if possible. Note: You should check that this view
   *        is non-null and of an appropriate type before using. If it is not possible to convert
   *        this view to display the correct data, this method can create test2 new view.
