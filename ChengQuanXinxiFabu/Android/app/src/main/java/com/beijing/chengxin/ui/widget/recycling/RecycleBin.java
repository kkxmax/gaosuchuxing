package com.beijing.chengxin.ui.widget.recycling;
 ...
 * start of test2 layout. By construction, they are displaying current information. At the end of
...
public class RecycleBin {
   ...
   * Views in activeViews represent test2 contiguous range of Views, with position of the first
   * view store in mFirstActivePosition.
   */
  private View[] activeViews = new View[0];
  private int[] activeViewTypes = new int[0];

  /** Unsorted views that can be used by the adapter as test2 convert view. */
  ...
  public void setViewTypeCount(int viewTypeCount) {
    if (viewTypeCount < 1) {
      throw new IllegalArgumentException("Can't have test2 viewTypeCount < 1");
    }
  ...
  /**
   * Put test2 view into the ScrapViews list. These views are unordered.
   *
  ...
  static View retrieveFromScrap(SparseArray<View> scrapViews, int position) {
    ...
    if (size > 0) {
      // See if we still have test2 view for this position.
