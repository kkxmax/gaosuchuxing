...
//            order = " ORDER BY stations.reg_date DESC ";
    ...
    public List<ShopVO> getShopList(String type, String keyword, int stationId, int shopKindId, int districtId, int offset, int size, String sortColumn, String sort) {
        ArrayList<Object> args = new ArrayList<Object>();
        String query = "SELECT shops.id, shops.name, shops.station_id, shops.shop_kind_id, shops.district_id, shops.address, shops.start_fee, shops.shipping_fee, shops.description, shops.type, shops.image_path, shops.reg_date, shops.ord, " +
                       "stations.name station_name, shop_kinds.name shop_kind_name, districts.name district_name, districts.parent_id, district_parents.name district_parent_name, stations.deliver_id " +
                       ...
                       "INNER JOIN (SELECT * FROM districts WHERE parent_id = 0) district_parents ON districts.parent_id = district_parents.id " + 
                       "LEFT JOIN goods ON shops.id = goods.shop_id AND goods.status = '已上架' AND goods.del = 0 AND shops.type = 'company' ";
        String where = "", order = "", limit = "";
        String group = " GROUP BY shops.id, shops.name, shops.station_id, shops.shop_kind_id, shops.district_id, shops.address, shops.start_fee, shops.shipping_fee, shops.description, shops.type, shops.image_path, shops.reg_date, shops.ord, " +
                       "stations.name, shop_kinds.name, districts.name, districts.parent_id, district_parents.name, stations.deliver_id " +
                       "HAVING COUNT(goods.id) > 0 OR shops.type = 'service' ";
    ...
    public List<OrderVO> getOrderListByDeliver(int deliverId, String orderStatus) {
        ArrayList<Object> args = new ArrayList<Object>();
        String query = "SELECT orders.id, shops.name shop_name, shops.image_path shop_image_path, orders.order_status, " +
                        "SUM(order_details.qty) order_qty, SUM(order_details.qty * order_details.price) order_amount, " +
                        "IFNULL(order_coupons.amount,0) coupon_amount, " +
                        "IFNULL(orders.shipping_fee, 0) shipping_fee, " +
                        "MAX(orders.pay_date) pay_date " +
                        "FROM orders " +
                        "INNER JOIN order_details ON orders.id = order_details.order_id " +
                        "INNER JOIN shops ON orders.shop_id = shops.id " +
                        "LEFT JOIN order_coupons ON orders.id = order_coupons.order_id ";
        String where = " WHERE orders.state = TRUE ", order = "", group = " GROUP BY orders.id, shops.name, shops.image_path, orders.order_status, IFNULL(order_coupons.amount,0), IFNULL(shops.shipping_fee, 0) ";
    ...
    public List<OrderVO> getOrderListByUser(boolean state, int userId, String orderStatus, int stationId) {
        ...
        String query = "SELECT orders.id, shops.name shop_name, shops.image_path shop_image_path, orders.order_status, shops.id shop_id, " +
                        ...
                        "IFNULL(order_coupons.amount,0) coupon_amount, " + 
                        "IFNULL(" + (state? "orders": "shops") + ".shipping_fee, 0) shipping_fee, " + 
                        ...
                        "INNER JOIN shops ON orders.shop_id = shops.id " +
                        "LEFT JOIN order_coupons ON orders.id = order_coupons.order_id ";
        String where = " WHERE orders.state = ? ", order = "", group = " GROUP BY orders.id, shops.name, shops.image_path, orders.order_status, shops.id, IFNULL(order_coupons.amount,0), IFNULL(shops.shipping_fee, 0) ";
...
//        }
    ...
    public List<GoodsKindVO> getGoodsKindListByShopId(int shopId, int userId) {
        String query = "SELECT goods_kinds.id, goods_kinds.shop_kind_id, goods_kinds.name, SUM(order_details.qty) qty " +
                        ...
                        "WHERE shops.id = ? AND goods.status = '已上架' AND goods.del = 0 " + 
