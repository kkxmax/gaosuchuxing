...
$(function() {
    $('.btn_minus').click(function() {
        ...
        setOrder($('#shopId').val(), id, -1, searchKey);
...
function onSearch() {
    ...
    if (searchKey == '')
        searchKey = 'NULL';
    onLoadGoodsList($('#shopId').val(), searchKey);
}
...
function onLoadGoodsList(_shopId, _searchKey) {
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            getOrderInfo($('#shopId').val(), _searchKey);
            ...
            setOrder($('#shopId').val(), id, -1, _searchKey);
...
function onGoodsModal(_goodsId) {
    $.ajax({
        url: "getGoodsInfo",
        cache: false,
        data: {goodsId: _goodsId, shopId: $('#shopId').val()},
...
//    alert($('#goodsId').val());
    ...
    setOrder($('#shopId').val(), goodsId, 1, searchKey);
        ...
        success: function(data) {
            if (data != null && data != 'empty') {
                ...
                if (amount >= startFee) {
                    ...
                    $('#btn-order-submit').addClass("des");
                    $('#btn-order-submit').removeAttr("onclick");
                    $('#btn-order-submit').text("差" + (startFee - amount) + "元起送");
                }
...
function onDeleteCart() {
    ...
    $.ajax({
       url: "deleteAllOrder" ,
       cache: false,
       data: {shopId: $('#shopId').val(), searchKey: _searchKey},
       success: function(_result) {
            $('#btn-order-submit').removeAttr("onclick");
