...
<input type="hidden" id="opinion" value="${opinion}"/>
<input type="hidden" id="predictTime" value="${predictTime}"/>
<c:choose>
            ...
            <div class="row init-div-horizontal">                
                <div class="col-xs-12 init-div-horizontal">
                    ...
                    <c:forEach var="coupon" items="${coupons}">
                        <fmt:formatDate pattern="yyyy.M.d" var="fmtValidFrom" value="${coupon.validFromDate}" />
                        <fmt:formatDate pattern="yyyy.M.d" var="fmtValidTo" value="${coupon.validToDate}" />
                        ...
                        <c:choose>
                            <c:when test="${coupon.couponType == 'full_type'}">
                                <c:if test="${not empty useCoupon}">
                                    <div class="row init-div-horizontal"  style="background: url('<c:url value="/assets/img/radius_back_body.png"/>'); height: 130px; margin-top: 20px;" onclick="onFullCoupon('${coupon.couponId}')">
                                </c:if>
                                    ...
                                    <div style="margin: 0px 15px 0 60px;">
                                        <div class="col-xs-12 init-div-horizontal" style="margin-top: 10px">
                                            <div class="col-xs-5 init-div-horizontal">
                                                <p class="custom-text-over-status" style="font-size:14px;color: #3E6DBB;font-weight: bold;">${coupon.couponKind}可用</p>    
                                            </div>
                                            <div class="col-xs-7 init-div-horizontal text-right">                                            
                                                <p class="custom-text-over-status" style="font-size:11px;color: #B7A6A6; margin-top: 5px;">${fmtValidFrom} 一 ${fmtValidTo}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-xs-12 init-div-horizontal" style="">
                                            <p class="custom-text-over-status" style="font-size:12px;color: #3E6DBB;">全品类优惠券，所有高点服务区可用</p>
                                        </div>
                                    </div>
                            </c:when>
                            ...
                            <c:otherwise>
                                <c:if test="${not empty useCoupon}">
                                    <div class="row init-div-horizontal"  style="background: url('<c:url value="/assets/img/radius_back_body.png"/>'); height: 130px; margin-top: 20px;" onclick="onUnLimitedCoupon('${coupon.couponId}');">
                                </c:if>
                                <c:if test="${empty useCoupon}">
                                    <div class="row init-div-horizontal"  style="background: url('<c:url value="/assets/img/radius_back_body.png"/>'); height: 130px; margin-top: 20px;">
                                </c:if>
                                    ...
                                    <div style="margin: 0px 15px 0 60px;">
                                        <div class="col-xs-12 init-div-horizontal" style="margin-top: 10px">
                                            <div class="col-xs-5 init-div-horizontal">
                                                <p class="custom-text-over-status" style="font-size:14px;color: #FF3E59;font-weight: bold;">满0元可用</p>
                                            </div>
                                            <div class="col-xs-7 init-div-horizontal text-right">
                                                <p class="custom-text-over-status" style="font-size:11px;color: #B7A6A6; margin-top: 5px;">${fmtValidFrom} 一 ${fmtValidTo}</p>
                                            </div>
                                        ...
                                        <div class="col-xs-12 init-div-horizontal" style="">
                                            <p class="custom-text-over-status" style="font-size:12px;color: #FF3E59;">全品类优惠券，所有高点服务区可用</p>
                                        </div>
                                    </div>
                            </c:otherwise>
                        </c:choose>    
