...
<div class="delivery container-fluid container-lf-space main-body init-div-horizontal">
    ...
    <div style="text-align: center; padding-top: 20px; /*background: url('<c:url value="/assets/img/adv_button.png"/>') no-repeat; background-size: contain; background-position: right; margin-right: 10px;*/">
        ...
        <a href="javascript:;" onclick="onShare();" id="shareCoupon">
            <img src="<c:url value="/assets/img/adv_button.png"/>" style="position: fixed; right: 35px; top: 195px; width: 45px; height: 45px;" >
        </a>
    </div>
    ...
    <div class="row init-div" style="margin-top: 10px;">
        <div style="width: 80%; margin: auto">
            <button type="button" class="btn btn-primary" style="width: 100%; border-radius: 17px !important; border:1px solid #3e6dbb; background: #fff; color: #3e6dbb;" onclick="location.href='<c:url value='/user/home'/>'">返回首页</button>
        </div>
    </div>
</div>
                    ...
                    <div class="row init-div">
                        <div class="col-xs-6 init-div">
                            <div style="text-align: center; width: 80px; margin: auto; margin-top: 75px; margin-bottom: 15px; ">
                                <c:choose>
                                    <c:when test="${not empty stationLat}">
                                        <a class="no_underline" href="javascript:;" onclick="onGoGaodeMap('${myLng}', '${myLat}', '${stationLng}', '${stationLat}', '${stationName}')" style="color: black; background: url('<c:url value="/assets/img/pay_icon_mapgaode@2x.png"/>') no-repeat; background-size: 50px 50px; padding-top: 60px; ">高德地图</a>
                                    </c:when>
                                    <c:otherwise>
                        ...
                        <div class="col-xs-6 init-div">
                            <div style="text-align: center; width: 80px; margin: auto; margin-top: 75px; margin-bottom: 15px; ">
                                <c:choose>
                                    <c:when test="${not empty stationLat}">
                                        <a class="no_underline" href="javascript:;" onclick="onGoBaiduMap('${myLat}','${myLng}','${stationLat}','${stationLng}','${stationName}','${region}')" style="color: black; background: url('<c:url value="/assets/img/pay_icon_mapbaidu@2x.png"/>') no-repeat; background-size: 50px 50px; padding-top: 60px; ">百度地图</a>
                                    </c:when>
                                    <c:otherwise>
                                        <a class="no_underline" href="javascript:;" style="color: black; background: url('<c:url value="/assets/img/pay_icon_mapbaidu@2x.png"/>') no-repeat; background-size: 50px 50px; padding-top: 60px; ">百度地图</a>
                                    </c:otherwise>
                                </c:choose>
                            </div>
                        </div>
...
<div class="modal fade" id="share-modal" tabindex="-1" aria-hidden="false">
    <div class="modal-dialog" style="margin-top: 3%" data-dismiss="modal">
        <img src="<c:url value="/assets/img/share.png"/>" style="width: 95%" >
    </div>
</div>        