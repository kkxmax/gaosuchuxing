...
public class AdminController extends MultiActionController {
        ...
        if (request.getMethod().equals("GET")) {
            ...
            try {
                ...
                if (loginUser != null && loginUser.getPassword().equals(loginForm.getDigest())) {
                    ...
                    List<PermissionVO> permissions = webDelegate.getManagerPermission(loginUser.getId(), -1);
                    List<PermissionVO> sysPermissions = webDelegate.getManagerPermission(loginUser.getId(), 10);
                    
                    session.setAttribute(Constant.SESSION_INFO.LOGIN_USER, loginUser);
                    session.setAttribute(Constant.SESSION_INFO.PERMISSIONS, permissions);
                    session.setAttribute(Constant.SESSION_INFO.SYS_PERMISSIONS, sysPermissions);
    ...
    public ModelAndView managerList(HttpServletRequest request, HttpServletResponse response, ManagerSearchForm searchForm) {
        ...
        ManagerVO loginUser = (ManagerVO)session.getAttribute(Constant.SESSION_INFO.LOGIN_USER);
        if (!webDelegate.checkPermission(loginUser.getId(), Constant.PERMISSION.ADMIN)) {
            return new ModelAndView("redirect:/admin/logout");
        }
...
//        }
    ...
    public ModelAndView groupList(HttpServletRequest request, HttpServletResponse response) {
        ...
        HttpSession session = request.getSession(false);
        ManagerVO loginUser = (ManagerVO)session.getAttribute(Constant.SESSION_INFO.LOGIN_USER);
        if (!webDelegate.checkPermission(loginUser.getId(), Constant.PERMISSION.GROUP)) {
            return new ModelAndView("redirect:/admin/logout");
        }
...
//        }
    ...
    public ModelAndView stationList(HttpServletRequest request, HttpServletResponse response, StationSearchForm searchForm) {
        ...
        HttpSession session = request.getSession(false);
        ManagerVO loginUser = (ManagerVO)session.getAttribute(Constant.SESSION_INFO.LOGIN_USER);
        if (!webDelegate.checkPermission(loginUser.getId(), Constant.PERMISSION.STATION)) {
            return new ModelAndView("redirect:/admin/logout");
        }
    ...
    public String getStation(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (station != null) {
            ...
            json.put("parentId", station.getDistrictParentId());
            json.put("districtId", station.getDistrictId());
    ...
    @RequestMapping(value = "/validateDeliverName")
    public String validateDeliverName(HttpServletRequest request, HttpServletResponse response) {
        int deliverId = NumberUtil.strToInt(request.getParameter("deliverId"));
        String deliverName = request.getParameter("deliverName");
        
        try {
            DeliverVO deliver = webDelegate.getDeliverByName(deliverName);
            if (deliver == null || deliver.getId() == deliverId)
                response.getOutputStream().write("success".getBytes());
            else
                response.getOutputStream().write("error".getBytes());
        } catch (Exception ex) {
    ...
    @RequestMapping(value = "/validateManagerName")
    public String validateManagerName(HttpServletRequest request, HttpServletResponse response) {
        int managerId = NumberUtil.strToInt(request.getParameter("managerId"));
        ...
        try {
            ManagerVO manager = webDelegate.getMangerByName(managerName);
...
//        }
    ...
    public ModelAndView stationFormPost(@PathVariable("id") String id, @RequestParam("file") MultipartFile file, HttpServletRequest request, HttpServletResponse response, StationVO stationForm) {
        ...
        int x = 0, y = 0, sw = 0, sh = 0, w = 0, h = 0;
        if (NumberUtil.strToInt(request.getParameter("w")) != -1) {
            isCrop = true;
            x = NumberUtil.strToInt(request.getParameter("x1"));
            y = NumberUtil.strToInt(request.getParameter("y1"));
            sw = NumberUtil.strToInt(request.getParameter("sw"));
            sh = NumberUtil.strToInt(request.getParameter("sh"));
            w = NumberUtil.strToInt(request.getParameter("w"));
            h = NumberUtil.strToInt(request.getParameter("h"));
        }
        ...
        if (file != null && !file.getOriginalFilename().isEmpty() && file.getSize() > 0) {
            ...
            try {                    
                ...
                if (isCrop) {
                    double cropX = img.getWidth() * x / sw;
                    double cropY = img.getHeight() *y / sh;
                    double cropW = img.getWidth() * w / sw;
                    double cropH = img.getHeight() * h / sh;
                    
                    img = img.getSubimage((int)cropX, (int)cropY, (int)cropW, (int)cropH);
                }
...
//                out.flush();
    ...
    public ModelAndView shopList(@PathVariable("type") String type, HttpServletRequest request, HttpServletResponse response, ShopSearchForm searchForm) {
        ...
        HttpSession session = request.getSession(false);
        ManagerVO loginUser = (ManagerVO)session.getAttribute(Constant.SESSION_INFO.LOGIN_USER);
        if (!webDelegate.checkPermission(loginUser.getId(), Constant.PERMISSION.SHOP)) {
            return new ModelAndView("redirect:/admin/logout");
        }
        ...
        if (shops != null) {
            ...
            for (ShopVO shop: shops) {
                ...
                if (type.equals(Constant.SHOP_KIND.COMPANY)) {
                    ...
                    item.put("7", shop.getOrd());
                    item.put("8", DateUtil.formatDateTime(shop.getRegDate()));
                    ...
                    tag += "<a href='javascript:;' onclick=onOrd('" + shop.getId() + "') class='btn btn-xs blue'> 排序 <i class='fa fa-sort-amount-desc'></i></a>";
                    item.put("9", tag);
                } else {
                    item.put("5", shop.getOrd());
                    item.put("6", DateUtil.formatDateTime(shop.getRegDate()));
                    String tag = "<a href='" + request.getContextPath() + "/admin/shopShow/" + shop.getId() + "' class='btn btn-xs purple'> 查看 <i class='fa fa-file-text'></i></a>";
                    tag += "<a href='" + request.getContextPath() + "/admin/shopForm/" + shop.getId() + "' class='btn btn-xs blue'> 编辑 <i class='fa fa-edit'></i></a>";
                    tag += "<a href='javascript:;' onclick=onDeleteShop('" + shop.getId() + "') class='btn btn-xs grey-cascade'> 删除 <i class='fa fa-trash-o'></i></a>";
                    tag += "<a href='javascript:;' onclick=onOrd('" + shop.getId() + "') class='btn btn-xs blue'> 排序 <i class='fa fa-sort-amount-desc'></i></a>";
    ...
    @RequestMapping(value = "/getShopOrd")
    public String getShopOrd(HttpServletRequest request, HttpServletResponse response) {
        int shopId = NumberUtil.strToInt(request.getParameter("shopId"));
        ...
        try {
            ...
            if (shop != null)
                ord = shop.getOrd();
            response.getOutputStream().write(String.valueOf(ord).getBytes());
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        return null;
    ...
    @RequestMapping(value = "/updateShopOrd")
    public String updateShopOrd(HttpServletRequest request, HttpServletResponse response) {
        int shopId = NumberUtil.strToInt(request.getParameter("shopId"));
        int ord = NumberUtil.strToInt(request.getParameter("ord"), 0);
        
        try {
            webDelegate.updateShopOrd(shopId, ord);
            response.getOutputStream().write("success".getBytes());
        } catch (Exception ex) {
            ex.printStackTrace();
        }
...
//        String keyword = request.getParameter("keyword");
    ...
    public ModelAndView goodsKindList(HttpServletRequest request, HttpServletResponse response, GoodsKindSearchForm searchForm) {
        ...
        HttpSession session = request.getSession(false);
        ManagerVO loginUser = (ManagerVO)session.getAttribute(Constant.SESSION_INFO.LOGIN_USER);
        if (!webDelegate.checkPermission(loginUser.getId(), Constant.PERMISSION.GOODS_KIND)) {
            return new ModelAndView("redirect:/admin/logout");
        }
    ...
    public ModelAndView orderList(HttpServletRequest request, HttpServletResponse response, OrderSearchForm searchForm) {
        ...
        HttpSession session = request.getSession(false);
        ManagerVO loginUser = (ManagerVO)session.getAttribute(Constant.SESSION_INFO.LOGIN_USER);
        if (!webDelegate.checkPermission(loginUser.getId(), Constant.PERMISSION.ORDER)) {
            return new ModelAndView("redirect:/admin/logout");
        }
        ...
        if (searchKey != null) {
            if (searchKey.equals("orderNum"))
                orderNum = searchForm.getKeyword();
            else if (searchKey.equals("userName"))
                userName = searchForm.getKeyword();
            else
                deliverName = searchForm.getKeyword();
        }
        String orderStatus = searchForm.getOrderStatus();
        String from = searchForm.getFrom();
        String to = searchForm.getTo();
        
        double totalAmount = webDelegate.getOrderAmount(1, orderNum, userName, deliverName, orderStatus, from, to);
        mav.addObject("totalAmount", NumberUtil.format(totalAmount, "0.00"));
    ...
    public String getOrderList(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (orders != null) {
            ...
            for (OrderVO order: orders) {
                ...
                if ((order.getOrderAmount() + order.getShippingFee() - order.getCouponAmount()) > 0)
                    item.put("4", NumberUtil.format(order.getOrderAmount() + order.getShippingFee() - order.getCouponAmount()));
                else
                    item.put("4", "0.00");
    ...
    public ModelAndView orderShow(@PathVariable("id") String id, HttpServletRequest request, HttpServletResponse response, OrderVO orderForm) {
        ...
        ShopVO shop = webDelegate.getShop(orderForm.getShopId());
        StationVO station = webDelegate.getStation(shop.getStationId());
        ...
        double orderAmount = orderDetailAmount - orderCouponAmount + orderForm.getShippingFee();
        ...
        mav.addObject("totalAmount", NumberUtil.format(orderAmount));
    ...
    public ModelAndView statDeliver(HttpServletRequest request, HttpServletResponse response, OrderSearchForm searchForm) {
        ...
        HttpSession session = request.getSession(false);
        ManagerVO loginUser = (ManagerVO)session.getAttribute(Constant.SESSION_INFO.LOGIN_USER);
        if (!webDelegate.checkPermission(loginUser.getId(), Constant.PERMISSION.DELIVER_STAT)) {
            return new ModelAndView("redirect:/admin/logout");
        }
    ...
    public ModelAndView activityList(HttpServletRequest request, HttpServletResponse response, ActivitySearchForm searchForm) {
        ...
        HttpSession session = request.getSession(false);
        ManagerVO loginUser = (ManagerVO)session.getAttribute(Constant.SESSION_INFO.LOGIN_USER);
        if (!webDelegate.checkPermission(loginUser.getId(), Constant.PERMISSION.ACTIVITY)) {
            return new ModelAndView("redirect:/admin/logout");
        }
    ...
    public String getActivityList(HttpServletRequest request, HttpServletResponse response) {
        ...
        String sortColumn = "";
//        if (sortColumnId > 1)
        sortColumn = Constant.ACTIVITY_COLUMNS[sortColumnId];
        ...
        if (activities != null) {
            ...
            for (ActivityVO activity: activities) {
                ...
                item.put("6", DateUtil.formatDate(activity.getRegDate(), "yyyy-MM-dd HH:mm"));
...
//        }
    ...
    public ModelAndView activityFormPost(@PathVariable("id") String id, @RequestParam("file") MultipartFile file, HttpServletRequest request, HttpServletResponse response, ActivityVO activityForm) {
        ...
        activityForm.setShareTitle(CommonUtil.charsetEncoding(activityForm.getShareTitle()));
        activityForm.setShareContent(CommonUtil.charsetEncoding(activityForm.getShareContent()));        
    ...
    public String addTmpActivityDetail(HttpServletRequest request, HttpServletResponse response) {        
        ...
        if (!CommonUtil.isEmptyStr(name))
            tmp.setActivityName(name);
        else
            tmp.setActivityName("xxx");
    ...
    public ModelAndView feedbackList(HttpServletRequest request, HttpServletResponse response, FeedbackSearchForm searchForm) {
        ...
        HttpSession session = request.getSession(false);
        ManagerVO loginUser = (ManagerVO)session.getAttribute(Constant.SESSION_INFO.LOGIN_USER);
        if (!webDelegate.checkPermission(loginUser.getId(), Constant.PERMISSION.FEEDBACK)) {
            return new ModelAndView("redirect:/admin/logout");
        }
...
//        request.setAttribute("subTitle", "");
    ...
    public ModelAndView deliverList(HttpServletRequest request, HttpServletResponse response, DeliverSearchForm searchForm) {
        ...
        HttpSession session = request.getSession(false);
        ManagerVO loginUser = (ManagerVO)session.getAttribute(Constant.SESSION_INFO.LOGIN_USER);
        if (!webDelegate.checkPermission(loginUser.getId(), Constant.PERMISSION.DELIVER)) {
            return new ModelAndView("redirect:/admin/logout");
        }
...
//        request.setAttribute("subTitle", "");
    ...
    public String deleteDeliver(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            if (webDelegate.deleteDeliver(deliverId)) {
                response.getOutputStream().write("success".getBytes());
            } else {
                if (webDelegate.isDeliverOfStation(deliverId))
                    CommonUtil.sendMessageResponse(response, "该配送员已关联了服务区，先去更换服务区负责的配送员吧");
                else
                    CommonUtil.sendMessageResponse(response, "该配送员有配送数据，因此无法被删除");
            }
        } catch (Exception ex) {
            ex.printStackTrace();
        }
...
//        request.setAttribute("subTitle", "运营人员管理");
    ...
    public ModelAndView userList(HttpServletRequest request, HttpServletResponse response, UserSearchForm searchForm) {
        ...
        HttpSession session = request.getSession(false);
        ManagerVO loginUser = (ManagerVO)session.getAttribute(Constant.SESSION_INFO.LOGIN_USER);
        if (!webDelegate.checkPermission(loginUser.getId(), Constant.PERMISSION.USER)) {
            return new ModelAndView("redirect:/admin/logout");
        }
...
//        request.setAttribute("subTitle", "");
    ...
    public String getUserList(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (users != null) {            
            for (UserVO user: users) {
                ...
                if (user.getOrderAmount() > 0)
                    item.put("4", NumberUtil.format(user.getOrderAmount()));
                else
                    item.put("4", 0.00);
                datas.add(item);
            }
    ...
    public ModelAndView goodsList(HttpServletRequest request, HttpServletResponse response, GoodsSearchForm searchForm) {
        ...
        HttpSession session = request.getSession(false);
        ManagerVO loginUser = (ManagerVO)session.getAttribute(Constant.SESSION_INFO.LOGIN_USER);
        if (!webDelegate.checkPermission(loginUser.getId(), Constant.PERMISSION.GOODS)) {
            return new ModelAndView("redirect:/admin/logout");
        }
    ...
    public String validateGoodsName(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            if (!webDelegate.checkGoodsByName(goodsName)) {
                response.getOutputStream().write("success".getBytes());
            } else  {
                int checkId = webDelegate.getGoodsIdForCheckGoodsNameOfShop(shopId, goodsName);
                
                if (checkId == 0 || checkId == goodsId)
                    response.getOutputStream().write("success".getBytes());
                else
                    response.getOutputStream().write("error".getBytes());
            }    
...
//                out.flush();
    ...
    public ModelAndView activityNoticeList(HttpServletRequest request, HttpServletResponse response) {
        ...
        HttpSession session = request.getSession(false);
        ManagerVO loginUser = (ManagerVO)session.getAttribute(Constant.SESSION_INFO.LOGIN_USER);
        if (!webDelegate.checkPermission(loginUser.getId(), Constant.PERMISSION.ACTIVTY_NOTICE)) {
            return new ModelAndView("redirect:/admin/logout");
        }
    ...
    public ModelAndView activityNoticeFormPost(@PathVariable("id") String id, @RequestParam("file") MultipartFile file, HttpServletRequest request, HttpServletResponse response, ActivityNoticeVO activityNoticeForm) {
        ...
        activityNoticeForm.setShareTitle(CommonUtil.charsetEncoding(activityNoticeForm.getShareTitle()));
        activityNoticeForm.setShareContent(CommonUtil.charsetEncoding(activityNoticeForm.getShareContent()));
    ...
    public ModelAndView userCouponList(HttpServletRequest request, HttpServletResponse response, UserCouponSearchForm searchForm) {
        ...
        HttpSession session = request.getSession(false);
        ManagerVO loginUser = (ManagerVO)session.getAttribute(Constant.SESSION_INFO.LOGIN_USER);
        if (!webDelegate.checkPermission(loginUser.getId(), Constant.PERMISSION.COUPON)) {
            return new ModelAndView("redirect:/admin/logout");
        }
...
//                item.put("0", ++num);
                item.put("0", userCoupon.getCouponNum());
                item.put("1", userCoupon.getCouponPrice());
    ...
    public String checkFeedback(HttpServletRequest request, HttpServletResponse response) {
        boolean status = webDelegate.checkFeedback();
        if (status)
            CommonUtil.sendMessageResponse(response, "ok");
        else
            CommonUtil.sendMessageResponse(response, "cancel");
