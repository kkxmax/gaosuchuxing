...
$(function() {
    ...
    $('#stationId').live('change', function() {
        var stationId = $(this).val();
        $.ajax({
            cache: false,
            url: $('#get-station-url').val(),
            data: {stationId: stationId},
            success: function (_data) {
                        try {
                            var json = eval(_data);
                            var parentId = json.parentId;
                            var districtId = json.districtId;

                            $('#districtParentId option[value=' + parentId + ']').attr('selected','selected');
                            ...
                            $.ajax({
                                ...
                                success: function (_html) {
                                    $('.district-opt').empty().append(_html);
                                    $('#districtId option[value=' + districtId + ']').attr('selected','selected');
                                }
                            });

                        } catch (e) {
                            $('#districtParentId option[value="-2"]').attr('selected','selected');
                            var select = "<select id=\"districtId\" name=\"districtId\" class=\"form-control\"></select>";
                            $('.district-opt').empty().append(select);
                        }
            }
        })
    });
...
function showCoords(c) {
    ...
    $('#w').val(c.w);
    $('#h').val(c.h);
    $('#sw').val($('#img_preivew').width());
    $('#sh').val($('#img_preivew').height());
};
